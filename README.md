...
Please note that not all the source code has been uploaded, however, feel free to review the available code (which represents almost 90% of the backend), and please read the section *"About the trained model and results"* to gain better understanding of the project's capabilities.  
...
For this implementation, at each iteration the ROC curve was calculated to obtain the False Positive Rates (FPR) and True Positive Rates (TPR), then scipy's Brent's root-finding algorithm \cite{brent} implementation was used to find the root of a lambda function that calculates the difference between FPR and $1$ - TPR (which is FRR) at a given $x: x \in [0, 1]$. This root represents the threshold at which the FPR and FRR ($1$ - TPR) are equal, giving the EER value. 
