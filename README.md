...
For this implementation, at each iteration the ROC curve was calculated to obtain the False Positive Rates (FPR) and True Positive Rates (TPR), then scipy's Brent's root-finding algorithm implementation was used to find the root of a lambda function that calculates the difference between FPR and $1$ - TPR (which is FRR) at a given $x: x \in [0, 1]$. This root represents the threshold at which the FPR and FRR ($1$ - TPR) are equal, giving the EER value. 
